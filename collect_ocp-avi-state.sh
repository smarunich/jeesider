...
SSH_COLLECTION_CMDS="
hostname;docker info;iptables -nvL;iptables -nvL -t nat;ip route show table all;ifconfig;ip link;ip addr;sysctl -a;df -h;date;ntpq -p;ntpq -p;ping -c5 $AVI_CONTROLLER"
for NODE in $NODES_LIST; do
    ssh -oStrictHostKeyChecking=no $NODE $SSH_COLLECTION_CMDS &> $DEST/ssh-$NODE-info.txt
    #SE_CONTAINER_NAME=`docker ps | grep avi | awk '{printf $10"\n"}'`
    ssh -oStrictHostKeyChecking=no $NODE /bin/bash -c "docker exec -i $(docker ps | grep avi | awk '{printf $10"\n"}') /bin/bash -c "$SSH_COLLECTION_CMDS"" &> $DEST/ssh-se-$NODE-info.txt
done
